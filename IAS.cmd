...
echo 该项�?仅支�? Windows 7/8/8.1/10/11 及其服务器版�?.
goto done2
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 在系统中找不�? powershell.exe.
goto done2
)
...
%eline%
echo 脚本�?从临时文件夹�?�?动的.
echo 很可能您�?直接从存档文件中运�?�脚�?.
echo:
echo 提取存档文件，并从提取的文件夹中�?动脚�?.
goto done2
)
)
...
echo:
echo PowerShell无法正常工作。终止操作�?
echo 如果您�?�PowerShell应用了限�?,请撤销这些更改.
echo:
echo 请�?�查这�?页面以获取帮�?. %mas%IAS-Help#troubleshoot
goto done2
)
...
%eline%
echo 这个脚本需要�?�理员权限�?
echo 要这样做，�?�右�?单击此脚�?，然后选择“以管理员身份运行�?.
goto done2
)
...
echo WMI�?能工作。中�?...
echo:
echo 请查看�?�页�?以获取帮助�?%mas%IAS-Help#troubleshoot
goto done2
)
...
echo 用户账户SID�?找到。中�?...
echo:
echo 请�?�查�?�页�?以获取帮助�?%mas%IAS-Help#troubleshoot
goto done2
)
...
echo:
echo:
call :_color2 %_White% "             " %_Green% "构建 By Piash"
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Red% "点击�?盘上的[1,2,3,4,5,0]以操控菜�?"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo %line%
echo:
echo      一些用户的激活出现问�?,IDM�?能会显示虚假的序列号提示窗口.
echo:
call :_color2 %_White% "     " %_Green% "建�??使用“冻结试用”选项."
echo %line%
echo:
choice /C:19 /N /M ">    [1] 返回 [9] 激�? : "
...
echo:
if not exist "%IDMan%" (
call :_color %Red% "IDM �?安�??."
echo 你可以点击这里下�?  https://www.internetdownloadmanager.com/download.html
goto done
)
...
call :_color %Red% "无法连接internetdownloadmanager.com,已中止操作�?"
goto done
)
call :_color %Gray% "internetdownloadmanager.com �? ping 命令失败�?"
echo:
)
...
echo 检查信�? - [%regwinos% ^| %fullbuild% ^| %regarch% ^| IDM: %IDMver%]
...
echo:
echo 在创�? CLSID 注册表键的�?�份 %SystemRoot%\Temp
...
echo:
call :_color %Gray% "如果出现假序列号屏幕，�?�使用冻结试用选项�?"
) else (
call :_color %Green% "IDM �? 30 天试用期已成功永久冻结�?"
echo:
call :_color %Gray% "如果 IDM 弹出注册窗口，�?�重新安�? IDM�?"
)
...
if defined terminal (
call :_color %_Yellow% "按下0�?返回..."
choice /c 0 /n
) else (
call :_color %_Yellow% "按下任意�?返回..."
pause %nul1%
)
goto MainMenu
...
foreach ($regPath in $regPaths) {
	...
	Write-Host
	Write-Host "�? $regPath �?搜索 IDM CLSID 注册表键 "
	Write-Host
...
if (($finalValues.Count -gt 20) -and ($toggle -ne $null)) {
	$lockKey = $null
	$deleteKey = 1
    Write-Host "IDM 注册表的数量超过 20 �?。现在删除它�?，而不�?锁定它们..."
	Write-Host
}
